...
# install wmdatamining thread only following machine
# one production, ane testbed
if  HOST.startswith("vocms136") or HOST.startswith("vocms133"):
...
if  HOST.startswith("vocms127") or HOST.startswith("reqmgr2"):
    ...
    #wmdatamining.reqmgrdb_url = "%s/%s" % (data.couch_host, data.couch_reqmgr_db)
    wmdatamining.wmdatamining_url = "%s/%s" % (data.couch_host, data.couch_wmdatamining_db)
    wmdatamining.activeDuration = 60 * 5  # every 5 min
    wmdatamining.archiveDuration = 60 * 30 # every 15 min
