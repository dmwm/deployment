...
ecallayout(dqmitems, 'Ecal/Layouts/00 Overview/07 Trigger Primitives',
	   [{'path': 'EcalBarrel/EBSummaryClient/EBTTT emulator error quality summary', 'description': 'Summary of emulator matching quality. A tower is red if the number of events with Et emulation error is greater than 0.1 of total events. Towers with entries less than 3 are not considered. Also, an entire SuperModule can show red if its (data) Trigger Primitive digi occupancy is less than 5sigma of the overall SuperModule mean, or if more than 80% of its Trigger Towers are showing any number of TT Flag-Readout Mismatch errors.'}],
...
ecallayout(dqmitems, 'Ecal/Layouts/03 Noise/04 EB Rechit Occupancy L vs R',
	   [{'path': 'EcalBarrel/EBOccupancyTask/EBOT rec hit thr occupancy z- vs z+', 'description': 'Filtered rechit occupancy z- vs. z+.'}])
ecallayout(dqmitems, 'Ecal/Layouts/03 Noise/04 EE Rechit Occupancy z- vs z+',
	   [{'path': 'EcalEndcap/EEOccupancyTask/EEOT rec hit thr occupancy z- vs z+', 'description': 'Filtered rechit occupancy z- vs. z+.'}])
ecallayout(dqmitems, 'Ecal/Layouts/03 Noise/05 Rechit Occupancy z+ - z-',
	   [{'path': 'EcalBarrel/EBOccupancyTask/EBOT rec hit thr occupancy z+ - z-', 'description': 'Filtered rechit occupancy z+ - z-.'}],
	   [{'path': 'EcalEndcap/EEOccupancyTask/EEOT rec hit thr occupancy z+ - z-', 'description': 'Filtered rechit occupancy z+ - z-.'}])
# By SuperModule _______________
for (detector, label, maxchannel) in [('Endcap', 'EE', 9), ('Barrel', 'EB', 18)]: # Loop over EB,EE
  for sign in ['-', '+']: # Loop over z-side
    for channel in range(1, maxchannel+1): # Loop over channels
