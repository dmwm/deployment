def hltBTVlayout(i, p, *rows): i['HLT/Layouts/BTV/' + p] = DQMItem(layout=rows)


triggers =  ['BTagMu_DiJet/BTagMu_AK4DiJet20_Mu5'
...
for trigger in triggers:
    hltBTVlayout(dqmitems, trigger.split('/')[1],
        [{'path': 'HLT/BTV/{}/effic_bjetPt_1'.format(trigger),
            'description': 'efficiency vs bjet Pt',
            'draw': { 'withref': 'no', 'xmax': '650' }},
        ...
        {'path': 'HLT/BTV/{}/effic_bjetMulti'.format(trigger),
            ...
            'draw': { 'withref': 'no' }},
        #{'path': 'HLT/BTV/{}/globalEfficiencies'.format(trigger),
            #'description': 'global efficiencies from different histos',
            #'draw': { 'withref': 'no' }},
        {'path': 'HLT/BTV/{}/effic_bjetCSV_1'.format(trigger),
            'description': 'efficiency vs bjet1 CSV',
            'draw': { 'withref': 'no' }}
        ],
        [{'path': 'HLT/BTV/{}/effic_muPt_1'.format(trigger),
        ...
        {'path': 'HLT/BTV/{}/effic_muMulti'.format(trigger),
            ...
            'draw': { 'withref': 'no' }},
        #{ 'path': 'HLT/BTV/{}/effic_jetMulti'.format(trigger),
        #'description': 'efficieny vs jet multiplicity ',
        #'draw': { 'withref': 'no' }},
